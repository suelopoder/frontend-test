...
test('empty array renders no list', () => {
  ...
  const uploadText = getByText(/No documents/i);
...
test('list of one elem reders a doc', () => {
  const { getByText } = render(<Documents documents={[{ id: '1', name: 'A doc', size: 8000 }]} />);
  expect(getByText('A doc')).toBeInTheDocument();
});

test('list reders all docs', () => {
  const { getByText } = render(<Documents documents={[
    { id: '1', name: 'A doc', size: 8000 },
    { id: '2', name: 'Another doc', size: 9000 },
    { id: '3', name: 'And yet abother doc', size: 10000 },
  ]} />);
